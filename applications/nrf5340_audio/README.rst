...
.. note::
   In the current version of the application, bootloader is default disabled.
...
FOTA upgrades can be used to replace the application of app core and net core.
nRF5340 Audio application supports two type of DFU flash layouts, internal and external.
Internal supports only single image DFU.
External supports  :ref:`multi-image DFU <ug_nrf5340_multi_image_dfu>`.
LE Audio Controller Subsystem for nRF53 supports normal size and minimal size of bootloader.
buildprog.py will add overlay files depends on DFU type.

To enable internal flash DFU, one needs to add the following parameter to buildprog.py.

.. code-block:: console

   -m internal

To enable minimal size of net core bootloader, append one more parameter to buildprog.py.

.. code-block:: console

   -M

For example, to build the application with DFU internal flash layout enabled and net core bootloader minimal size by using the script for the application core with the ``debug`` application version for both the headset and the gateway, run the following command from the :file:`buildprog` directory:

.. code-block:: console

   python buildprog.py -c app -b debug -d both -m internal -M

To enable external flash DFU, one needs additional flash shield.
This reference application uses MX25R6435F as SPI NOR Flash.
See the following table for pin definition.
...
To enable external flash DFU, one needs to add the following parameter to buildprog.py.

.. code-block:: console

   -m external

For example, to build the application with DFU external flash layout enabled and net core bootloader minimal size by using the script for the application core with the ``debug`` application version for both the headset and the gateway, run the following command from the :file:`buildprog` directory:

.. code-block:: console

   python buildprog.py -c app -b debug -d both -m external -M
...
.. |net_core_hex_note| replace:: The network core for both gateway and headsets is programmed with the precompiled Bluetooth Low Energy Controller binary file :file:`ble5-ctr-rpmsg_<XYZ>.hex`, where ``<XYZ>`` corresponds to the controller version, for example :file:`ble5-ctr-rpmsg_3216.hex`.
   ...
   If DFU is enabled, Bluetooth Low Energy Controller binary file will be :file:`pcft_CPUNET.hex` .
