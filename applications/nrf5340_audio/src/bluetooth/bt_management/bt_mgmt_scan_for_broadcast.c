 ...
 *
 * @param	data		Advertising data to check and parse
 * @param	user_data	Will contain pointer to broadcast_source struct to be populated
 *
...
static bool scan_check_broadcast_source(struct bt_data *data, void *user_data)
{
	...
	if (data->type == BT_DATA_BROADCAST_NAME && data->data_len) {
		/* Ensure that broadcast name is at least one lower than BLE_SEARCH_NAME_MAX_LEN */
...
/**
 * @brief	Scan receive callback when scanning for broadcasters
 *
